class SleepRecord < ApplicationRecord
  ...
  def validate_no_overlap
    if user.present? && start_time.present? && end_time.present?
      conflicts = user.sleep_records.where.not(id: id).where("(start_time <= ?) and (end_time >= ?)", end_time, start_time)
      if conflicts.any?
  ...
  def one_sleep_record_per_day_per_user
    if user.present? && start_time.present?
      if user.sleep_records.where.not(id: id).where(start_time: start_time.beginning_of_day..start_time.end_of_day).count > 0
